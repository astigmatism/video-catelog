...
let idleHoverPreviewAuditStartedForCurrentIdleTransition = false;
...
sessionStore.onActivityStateChange((transition) => {
  ...
  if (transition.current.idle) {
    if (idleHoverPreviewAuditStartedForCurrentIdleTransition) {
      app.log.info(
        {
          event: 'hover_preview.idle_audit.already_started_for_idle_transition',
          ...sessionLifecyclePayload
        },
        'Idle hover preview audit was already started for the current idle transition.'
      );
      return;
    }

    idleHoverPreviewAuditStartedForCurrentIdleTransition = true;
  ...
  idleHoverPreviewAuditStartedForCurrentIdleTransition = false;
...
const HOVER_SPRITE_MAX_EXACT_LAYOUT_COLUMN_ROW_RATIO = 3;
const HOVER_SPRITE_LAYOUT = createHoverPreviewLayout(config.hoverPreviewFrameCount);
const HOVER_SPRITE_FRAME_COUNT = HOVER_SPRITE_LAYOUT.frameCount;
const HOVER_SPRITE_COLUMNS = HOVER_SPRITE_LAYOUT.columns;
const HOVER_SPRITE_ROWS = HOVER_SPRITE_LAYOUT.rows;
...
function normalizeHoverPreviewSpriteDurationSeconds(durationSeconds: number): number {
  if (!Number.isFinite(durationSeconds) || durationSeconds <= 0) {
    return HOVER_PREVIEW_MIN_CAPTURE_SECONDS;
  }

  return Math.max(HOVER_PREVIEW_MIN_CAPTURE_SECONDS, Number(durationSeconds.toFixed(3)));
}
...
function findBalancedExactHoverPreviewLayout(frameCount: number): HoverPreviewLayout | null {
  for (let rows = Math.floor(Math.sqrt(frameCount)); rows >= 1; rows -= 1) {
    ...
    if (columns / rows > HOVER_SPRITE_MAX_EXACT_LAYOUT_COLUMN_ROW_RATIO) {
      continue;
    }
...
function createHoverPreviewFilter(segmentCount: number, samplingFps: string): string {
  ...
  const tileFilter = `fps=${samplingFps},tile=${HOVER_SPRITE_COLUMNS}x${HOVER_SPRITE_ROWS}:nb_frames=${HOVER_SPRITE_FRAME_COUNT}:color=black[hover_preview_sprite]`;
...
async function generateHoverPreviewSprite(
  ...
  try {
    ...
    const finalItem = await updateCatalogItemProcessing(
      ...
      sessionId,
      {
        hoverPreviewSprite: createHoverPreviewSprite(outputDescriptor.relativePath, previewPlan.effectiveDurationSeconds),
    ...
    writePipelineLog(
      ...
      getCatalogItemLogContext(finalItem, sessionId),
      {
        ...
        hoverPreviewPlaybackDurationSeconds: normalizeHoverPreviewSpriteDurationSeconds(previewPlan.effectiveDurationSeconds),
        bookmarkCount,
        hoverPreviewRevision: bookmarkCount,
...
app.post('/api/catalog/:id/bookmarks', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  try {
    ...
    idleHoverPreviewRebuilder.requestCatalogItemHoverPreviewRebuild({
      itemId: item.id,
      reason: 'bookmark_added',
      bookmarkId: bookmark.id
    });
    reply.send({
      ok: true,
