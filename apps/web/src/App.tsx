...
function readStoredCatalogFilters(): CatalogFilters {
  const defaultFilters = getDefaultCatalogFilters();
  ...
  try {
    const rawValue = window.localStorage.getItem(CATALOG_FILTERS_STORAGE_KEY);
    if (!rawValue) {
      return defaultFilters;
    }
    ...
    const sortCategory = readString(parsedValue.sortCategory);
    const sortDirection = readString(parsedValue.sortDirection);

    return {
      search: readString(parsedValue.search) ?? defaultFilters.search,
      sortCategory: isCatalogSortCategory(sortCategory) ? sortCategory : defaultFilters.sortCategory,
      sortDirection: isCatalogSortDirection(sortDirection) ? sortDirection : defaultFilters.sortDirection,
      tagSearch: readString(parsedValue.tagSearch) ?? defaultFilters.tagSearch,
      selectedTagIds: normalizeStoredStringArray(parsedValue.selectedTagIds),
      randomSeed: normalizeCatalogRandomSeed(parsedValue.randomSeed)
    };
  } catch {
    return defaultFilters;
  }
...
function CatalogCard({
  ...
  const tagPopoverId = `tag-management-popover-${item.id}`;
...
export default function App(): JSX.Element {
  ...
  const filteredCatalog = useMemo(() => {
    ...
    const nextItems = catalog.filter((item) => {
  ...
  const catalogTagById = useMemo(() => {
    ...
    return tagsById;
  }, [availableTagOptions, tagFilterSuggestions]);
  ...
  return (
    <div className="app-shell">
      ...
      <main className={`app-main${isFilterDrawerOpen ? ' is-filter-drawer-open' : ''}`}>
        ...
        <section className="catalog-panel" aria-label="Catalog grid">
